...
export default {
  ...
  methods: {
            ...
            let downLineSlider =self.$refs.downLineSlider;
                ...
                if(self.actualNum){
                    for (let i = 0; i < self.rulerNumAtr.length; i++) {
                        console.log(self.rulerNumAtr[i].num+'||||'+self.actualNum);
    ...
    clacThinknessByTouch(pagey){
      ...
      buildShowRulerNum(pageY,clacHeaderHeigt){//根据滑动距离计算刻度值对应的  和业务相关哦显示值 ！！
      //目前逻辑 根据业务刻度定制化 划到某个区间即可完成对应显示值
    //   [0.6,0.7,0.9,1.2,1.6,2.1,2.8,3.4,4.8,6.4,8.8,9.5,11,12.7]
       
        // this.rulerNumAtr;

        return pageY-clacHeaderHeigt;
      },
      change(){
        ...
        if(this.nowPosionX2==this.max2){
            ...
            this.timerPosition2 = setTimeout(() => {
                    if(this.oldNowPosionX2!=this.nowPosionX2){
                        ...
                        this.buildSliderChangeData(this.nowPosionX2,'V_WELDING');
                    }
            }, 1000);
        }
    },
           ...
           console.log(self.pageBackTo)
            if(url=='/welding'){
                ...
                if(this.isModbusModal){
                    ...
                    let num =this.jinzhiChange2jinzhiFuc(`${bitInfoList.cut09}110${bitInfoList.cut07}${bitInfoList.cut06}${bitInfoList.cut05}${bitInfoList.cut04}${bitInfoList.cut03}${bitInfoList.cut02}${bitInfoList.cut01}`);
