<template>
  <div class="weldMMA" :class="[ifFixedFlag?'weldFixed':'',envType=='env_ios'?'env_ios_gClass':'']"  :style="{height:newContainHeight+20+'px'}" id="allPage" ref="allPage">
        <!-- <div class="header"><Icon type="ios-arrow-back" @click="go('/newIndex')"/>{{typeName}}<span class="setupyi">SET UP</span></div> -->
        <Head :wantTo="'/newIndex'" :typeName="typeName" :pageFrom="'/weld_mma'" :nowModalTypeId="nowModalTypeId"></Head>
        <div class="mmp" ref="mmpId" id="idid">
                ...
                <div class="electricCurrent">
                    <div class="up">
                        <div class="u-left"><img class="force"  src="../../assets/images/weld_icon_newForce.png"></div>
                        <div class="u-right">
            ...
            <div class="down thinkness"  v-if="downshowFlag && nowtypename=='THICKNESS'" ref="dth" :class="closeClass?'transdown':''">
                   ...
                   <div class="newContian" :style="{height:commonContainHeight+'px'}" ref="downLineSlider">
                    ...
                    <div class="b-t-btn-sub">
                        <div class="subbtn"  @click="newCloseModal('thinkness')">OK</div>
                    </div>
            </div>
  ...
  methods: {
           ...
           if(this.UnitFlag==1){
            ...
            downLineSlider.addEventListener('touchmove',(e)=>{ //屏幕滑动事件
             ...
             downLineSlider.addEventListener('touchend',(e)=>{ //屏幕滑动事件
                ...
                self.wtlLog('weld_mma',"winHeight="+self.winHeight+',dth='+dth);
       }else{
           if(this.UnitFlag==1){
               this.disNumAtr =[
                    {num:'19GA',height:8.05},
        ...
        if(this.nowPosionX==this.max){
            ...
            this.timerPosition1 = setTimeout(() => {
                    if(this.nowPosionX!=this.oldPosionX){
                        ...
                        this.buildSliderChangeData(this.nowPosionX,'FORCE');
                    }
            }, 1000);
        }
    },
        ...
        if(this.nowPosionX2>=this.max2){
            ...
            this.timerPosition2 = setTimeout(() => {
                    if(this.oldNowPosionX2!=this.nowPosionX2){
                        ...
                        this.buildSliderChangeData(this.nowPosionX2,'MMA_CURRENT');
                    }
            }, 1000);
        }
    },
    delFuc2(){
         if(this.nowPosionX2<=this.min2){
             this.nowPosionX2 =this.min2;
            return;
        }else{
            this.nowPosionX2=Math.round((this.nowPosionX2-this.paramIncreaseDistance2)*10)/10;
            let sliderBtn =  this.$refs.sbtn2
            //00、初始化滚动条位置 占比是百分比
            sliderBtn.style.left = (this.nowPosionX2-this.min2)/this.block2*100+'%';
            clearTimeout(this.timerPosition2);
    ...
    initElecticCurrent(){
                ...
                this.nowPosionX=Math.round(Math.round(this.block*(Math.round(myWidth)/100)*10)/10+this.min);//四舍五入保留一位小数
  ...
  watch: {
        getAndriodNewMsg(val, oldVal){
            if(val!=this.$store.state.AdroidOldMsg){
                ...
                this.wtlLog('weld_mma_bfa3','this.modelType'+this.modelType);
